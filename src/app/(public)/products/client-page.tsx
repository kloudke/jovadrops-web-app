...
function ProductCard({ product }: { product: Product }) {
  return (
    <div className="flex flex-col items-center p-6 bg-white rounded-2xl border border-slate-100 hover:shadow-xl hover:border-brand-primary/20 transition-all duration-300 group">
      ...
      <Link href={`/products/${product.slug}`} className="w-full aspect-[4/5] bg-brand-light/30 rounded-xl mb-6 relative flex items-center justify-center p-4 cursor-pointer block overflow-hidden">
        {product.image ? (
           <Image
             src={product.image}
             alt={product.name}
      ...
      <div className="text-center w-full">
        <Link href={`/products/${product.slug}`} className="hover:text-brand-primary transition-colors block">
